export module use {
	...
	export type Num<T extends "I" | "U" | "F", Size extends NumSize<T>> = T extends "U" | "I"
		? (Size extends 1 | 2 | 4 ? number : bigint)
	...
	export class Decoder {
		...
		num<T extends "I" | "U" | "F", Size extends NumSize<T>>(type: T, size: Size) {
			return () => {
				if (size == 1) { return (type == "U" ? this.u8() : this.i8()) as Num<T, Size>; }
				if (type == "F") { return ((size == 4) ? this.f32() : this.f64()) as Num<T, Size> }
				let num = 0n;
				let shift = 0n;
				while (true) {
					let byte = this.u8();
		...
		bool() { return !!this.u8() }
		char() { return String.fromCharCode(this.num("U", 4)()); }
		str() {
	...
	export class BufWriter implements Write {
		...
		char(char: string) { this.num("U", 4)(char.charCodeAt(0)) }
